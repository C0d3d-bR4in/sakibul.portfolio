...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "opacity-0 -translate-y-full pointer-events-none" : "py-5 bg-transparent"
          }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <button onClick={() => scrollTo("hero")} className="text-2xl font-bold font-mono tracking-tight group">
            ...
            <span className="text-foreground group-hover:text-primary transition-colors">Sakibul.Me</span>
            <span className="text-primary glow-text">/&gt;</span>
          </button>
          ...
          <div className="hidden md:flex items-center gap-1">
            {sections.filter((s) => s !== "hero").map((id) => (
              <button
                key={id}
                onClick={() => scrollTo(id)}
                className={`px-4 py-2 rounded-lg text-[15px] font-medium transition-all duration-300 ${active === id
          ...
          <div className="md:hidden glass mt-2 mx-4 rounded-xl p-4 animate-scale-in">
            {sections.filter((s) => s !== "hero").map((id) => (
              <button
                key={id}
                onClick={() => scrollTo(id)}
                className={`block w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-all ${active === id
                  ? "text-primary bg-primary/10"
      ...
      <nav
        className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 ${scrolled ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"
          }`}
      >
            ...
            .map((id) => {
              ...
              return (
                <button
                  key={id}
                  onClick={() => scrollTo(id)}
                  className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${active === id
